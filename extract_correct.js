...
for (let i = headerIndex + 1; i < rawData.length; i++) {
  ...
  const depth = String(row[depthIdx]);
  const dataType = String(row[dataTypeIdx]);
  const useYn = String(row[useYnIdx]);
  const pGroupCode = row[pGroupCodeIdx];
  const groupCode = row[groupCodeIdx];
  const groupName = row[groupNameIdx];

  // 필터링: DEPTH="2", DATA_TYPE="G", USE_YN="1" (문자열 비교)
  if (depth === '2' && dataType === 'G' && useYn === '1') {
    ...
    // GROUP_CODE의 언더스코어를 하이픈으로 변환
    const code = String(groupCode).replace(/_/g, '-');
    const name = groupName ? String(groupName) : code;
