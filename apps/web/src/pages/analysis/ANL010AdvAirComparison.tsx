...
export default function ANL010AdvAirComparison() {
  ...
  const q0 = useQuery<TrendResult>({
    queryKey: ['anl010', s0?.facilityId, r0.start, r0.end],
    queryFn: () => getFacilityTrendData(s0!.facilityId, r0.start, r0.end, '1s', 'air') as Promise<TrendResult>,
    enabled: !!s0,
  });
  const q1 = useQuery<TrendResult>({
    queryKey: ['anl010', s1?.facilityId, r1.start, r1.end],
    queryFn: () => getFacilityTrendData(s1!.facilityId, r1.start, r1.end, '1s', 'air') as Promise<TrendResult>,
    enabled: !!s1,
  });
  const q2 = useQuery<TrendResult>({
    queryKey: ['anl010', s2?.facilityId, r2.start, r2.end],
    queryFn: () => getFacilityTrendData(s2!.facilityId, r2.start, r2.end, '1s', 'air') as Promise<TrendResult>,
    enabled: !!s2,
  });
  const q3 = useQuery<TrendResult>({
    queryKey: ['anl010', s3?.facilityId, r3.start, r3.end],
    queryFn: () => getFacilityTrendData(s3!.facilityId, r3.start, r3.end, '1s', 'air') as Promise<TrendResult>,
    enabled: !!s3,
  });
  ...
  return (
    <div className="flex flex-col gap-4 h-full">
      <PageHeader
        title="에어 상세 비교"
        description="설비별 에어 순시값 상세 비교 | 해상도: 1초 | 경과시간 정렬"
      />
