...
export default function ANL007PeriodPowerComparison() {
  // ── 트리 상태 (단일 선택) ──
  ...
  const q0 = useQuery<TrendResult>({
    queryKey: ['anl007', facilityId, r0.start, r0.end, currentInterval],
    queryFn: () => getFacilityTrendData(facilityId!, r0.start, r0.end, currentInterval as '10s' | '1s', 'elec') as Promise<TrendResult>,
    enabled: !!facilityId && dates.length > 0,
  });
  const q1 = useQuery<TrendResult>({
    queryKey: ['anl007', facilityId, r1.start, r1.end, currentInterval],
    queryFn: () => getFacilityTrendData(facilityId!, r1.start, r1.end, currentInterval as '10s' | '1s', 'elec') as Promise<TrendResult>,
    enabled: !!facilityId && dates.length > 1,
  });
  const q2 = useQuery<TrendResult>({
    queryKey: ['anl007', facilityId, r2.start, r2.end, currentInterval],
    queryFn: () => getFacilityTrendData(facilityId!, r2.start, r2.end, currentInterval as '10s' | '1s', 'elec') as Promise<TrendResult>,
    enabled: !!facilityId && dates.length > 2,
  });
  const q3 = useQuery<TrendResult>({
    queryKey: ['anl007', facilityId, r3.start, r3.end, currentInterval],
    queryFn: () => getFacilityTrendData(facilityId!, r3.start, r3.end, currentInterval as '10s' | '1s', 'elec') as Promise<TrendResult>,
    enabled: !!facilityId && dates.length > 3,
  });
  const q4 = useQuery<TrendResult>({
    queryKey: ['anl007', facilityId, r4.start, r4.end, currentInterval],
    queryFn: () => getFacilityTrendData(facilityId!, r4.start, r4.end, currentInterval as '10s' | '1s', 'elec') as Promise<TrendResult>,
    enabled: !!facilityId && dates.length > 4,
  });
  const q5 = useQuery<TrendResult>({
    queryKey: ['anl007', facilityId, r5.start, r5.end, currentInterval],
    queryFn: () => getFacilityTrendData(facilityId!, r5.start, r5.end, currentInterval as '10s' | '1s', 'elec') as Promise<TrendResult>,
  ...
  const handleZoomRaw = useCallback((zoomRatio: number, timeRange?: { start: string; end: string }) => {
    ...
    try {
      if (timeRange) {
        // xLabels가 "HH:mm:ss" 형식이므로 그대로 저장
  ...
  return (
    <div className="flex flex-col gap-4 h-full">
      <PageHeader
        title="기간별 전력 상세 비교"
        description={`동일 설비의 날짜별 전력 순시값 비교 | 해상도: ${formatInterval(currentInterval)}`}
      />
      ...
      <div className="flex gap-3 flex-1 min-h-0">
        ...
        <div className="flex-1 flex flex-col gap-3 min-h-0">
          ...
          {/* 차트 */}
          <ChartCard
            title={!facilityId ? '전력 순시값 트렌드' : `${facilityId} 기간별 비교 (${dates.length}개 날짜)`}
            subtitle={`해상도: ${formatInterval(currentInterval)}`}
            className="flex-1 min-h-0"
            chartId="anl007-chart"
            exportData={chartData}
            exportFilename="기간별전력상세비교"
            minHeight={0}
            actions={
              <DynamicZoomBar
                isZoomed={isZoomed}
